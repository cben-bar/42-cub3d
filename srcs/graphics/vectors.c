...
/*   Updated: 2022/11/25 13:25:21 by mgolinva         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
		if (data->pov.direction_angle < 90 || data->pov.direction_angle > 270)
		{
			tmp_vec.start.fy = vector->direction.fy;
			tmp_vec.start.fx = vector->direction.fx;
			tmp_vec.direction.fx = tmp_vec.start.fx - 1;
			tmp_vec.dx = 1;
			tmp_vec.dy = tmp_vec.dx * tan(get_angle(data->pov.direction_angle));
			vector->step_x = tmp_vec.dx;
			vector->step_y = tmp_vec.dy;
		}
		else if (data->pov.direction_angle > 90 && data->pov.direction_angle < 270)
		{
			tmp_vec.start.fy = vector->direction.fy;
			tmp_vec.start.fx = vector->direction.fx;
			tmp_vec.direction.fx = tmp_vec.start.fx + 1;
...
{
		if (data->pov.direction_angle > 180 && data->pov.direction_angle < 360)
		{
			tmp_vec.start.fy = vector->direction.fy;
			tmp_vec.start.fx = vector->direction.fx;
			tmp_vec.direction.fy = tmp_vec.start.fy + 1;
			tmp_vec.dy = 1;
			tmp_vec.dx = tmp_vec.dy / tan(get_angle(data->pov.direction_angle));
			vector->step_x = tmp_vec.dx;
			vector->step_y = tmp_vec.dy;
		}
		if (data->pov.direction_angle > 0 && data->pov.direction_angle < 180)
		{
			tmp_vec.start.fy = vector->direction.fy;
			tmp_vec.start.fx = vector->direction.fx;
			tmp_vec.direction.fy = tmp_vec.start.fy - 1;
...
void	get_vector_step(t_data *data, t_vector *vector, int marker)
{
	...
	else if (data->pov.direction_angle == 0 ||  data->pov.direction_angle == 180)
	{
		vector->step_x = 1;
		vector->step_y = 0;
		return ;
...
t_bool	get_next_vertical_line(t_data *data, t_vector *vector, t_fcoo starting_coo)
{
	...
	if (data->pov.direction_angle < 90 || data->pov.direction_angle > 270)
	{
		...
		vector->dy = vector->dx * tan(get_angle(data->pov.direction_angle));
	}
	else if (data->pov.direction_angle > 90 && data->pov.direction_angle < 270)
	{
...
t_bool	get_next_horizontal_line(t_data *data, t_vector *vector, t_fcoo starting_coo)
{
	...
	if (data->pov.direction_angle > 180 && data->pov.direction_angle < 360)
	{
		...
		vector->dx = vector->dy / tan(get_angle(data->pov.direction_angle));
	}
	else if (data->pov.direction_angle > 0 && data->pov.direction_angle < 180)
	{
